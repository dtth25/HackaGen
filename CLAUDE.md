...
- **AI**: OpenRouter mặc định, paid. Book/Vid dùng `google/gemini-2.5-pro`, Slide/Quiz có thể override riêng (`OPENROUTER_{BOOK,SLIDE,QUIZ,VID}_MODEL`). Mỗi call retry đúng model đã chọn một lần khi provider/schema lỗi, không chuyển sang model khác; frontend không biết nhà cung cấp.
- **OCR/Embedding local (tuỳ chọn, cost-cutting)**: `OCR_PROVIDER=paddleocr` (local, CPU, free — `app/services/ocr.py`) và `EMBEDDING_PROVIDER=local` (bge-m3 qua sentence-transformers, CPU, free — `app/services/vector_store.py:LocalEmbeddingFunction`) là lựa chọn thay thế OpenRouter, mặc định vẫn `openrouter`. Verify thật 2026-07-19 (Windows dev, chưa build Docker Linux thật): PaddleOCR cần `enable_mkldnn=False` để tránh crash trên ít nhất 1 CPU thật, ~20-25s/trang CJK dày, ~620MB RSS. bge-m3 ~2.2GB tải lần đầu, ~260s cold load, ~1-2GB RAM. KHÔNG tự động fallback sang OpenRouter khi local lỗi (fail loud, tránh tốn tiền âm thầm).
