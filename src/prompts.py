...
SYLLABUS_ONLY_PROMPT = """\
BẠN LÀ CHUYÊN GIA THIẾT KẾ CHƯƠNG TRÌNH ĐÀO TẠO.
NHIỆM VỤ: Phân tích [CONTEXT] và tạo Khung chương trình (Syllabus) cho đối tượng "{target_audience}".

⚠️ QUY TẮC NGHIÊM NGẶT:
1. CHỈ TRẢ VỀ KHUNG XƯƠNG: Không viết nội dung bài giảng chi tiết ở bước này.
2. BỎ QUA: Trang bìa, tác giả, lời nói đầu, mục lục cũ của sách.
3. CẤU TRÚC: Chia từ 4-6 chương, mỗi chương 2-3 bài học.
4. ĐỊNH DẠNG: Chỉ xuất RAW JSON.

JSON SCHEMA YÊU CẦU:
{{
  "course_title": "Tên khóa học",
  "overview": "Mô tả tổng quan ngắn gọn (2 câu)",
  "chapters": [
    {{
      "chapter_index": 1,
      ...
      "lessons": [
        {{
          "lesson_index": 1,
          "lesson_title": "Tên bài học cụ thể",
          "summary": "Mục tiêu bài học này là gì (1 câu)"
        }}
      ]
    }}
  ]
...
BẠN LÀ CHUYÊN GIA BIÊN SOẠN SÁCH GIÁO KHOA.
NHIỆM VỤ: Dựa vào [CONTEXT], hãy viết nội dung chi tiết cho Chương: "{chapter_title}".

YÊU CẦU:
- Viết bài giảng sâu sắc cho từng bài học trong chương này.
- Mỗi bài học phải có: lecture (bài giảng chi tiết), key_points (5 ý chính), và assessment (3 câu hỏi).
- TUYỆT ĐỐI KHÔNG dùng dấu ngoặc kép lồng nhau.
- Định dạng trả về: RAW JSON.
...
{{
  ...
  "lessons": [
    {{
      "lesson_title": "...",
      "lecture": "Nội dung cực kỳ chi tiết...",
      "key_points": ["...", "...", "...", "...", "..."],
      "assessment": ["...", "...", "..."]
    }}
  ]
...
BẠN LÀ CHUYÊN GIA THIẾT KẾ CHƯƠNG TRÌNH ĐÀO TẠO CẤP CAO VÀ KỸ SƯ XỬ LÝ DỮ LIỆU SƯ PHẠM.
NHIỆM VỤ: Phân tích [CONTEXT] để xây dựng một khóa học hoàn chỉnh, có chiều sâu dành cho đối tượng "{target_audience}".
YÊU CẦU BỔ SUNG: {user_prompt}

⚠️ QUY TẮC VỆ SINH DỮ LIỆU & CHỐNG RÁC OCR (CỰC KỲ NGHIÊM NGẶT):
1. KHÔNG THAM CHIẾU FILE & SỐ TRANG: Tuyệt đối KHÔNG được đưa tên tệp tin (ví dụ: các chuỗi số kết thúc bằng .pdf, .docx), số trang (ví dụ: "Trang 1", "Trang 12"), hoặc các ký hiệu phân mục tài liệu gốc (ví dụ: "Phần 1.1") vào bất kỳ đâu trong nội dung bài học.
...
3. CHẶN ĐỨNG VIỆC BIẾN TÊN NGƯỜI THÀNH KHÁI NIỆM: Tên tác giả, dịch giả, ban biên tập (ví dụ: Nguyễn Văn Thu, Vũ Văn Hùng, Bùi Gia Thịnh, Phạm Kim Chung...) CHỈ là thông tin nhân sự. TUYỆT ĐỐI không sử dụng các tên riêng này để giải thích khái niệm học thuật (Không tạo ra các khái niệm kỳ dị như "Nguyên", "Thịnh", "Chung").
4. NGUYÊN TẮC TỰ ĐỨNG ĐỘC LẬP: Khóa học phải là một sản phẩm hoàn chỉnh, tự giải thích kiến thức một cách khoa học. KHÔNG được viết hướng dẫn kiểu "Người học cần đọc kỹ đoạn gốc để hiểu khái niệm". Bạn phải tự định nghĩa khái niệm đó dựa trên kiến thức học thuật.

⚠️ QUY TẮC AN TOÀN TRÁNH LỖI PARSE JSON (BẮT BUỘC):
1. KHÔNG XUỐNG DÒNG VẬT LÝ: Trong toàn bộ các trường text (như "description", "lecture"), TUYỆT ĐỐI không nhấn Enter xuống dòng vật lý. Nếu muốn ngắt dòng, bạn bắt buộc phải dùng ký tự viết tắt "\\n" (hai ký tự: gạch chéo ngược và chữ n).
2. TRÁNH DÙNG NGOẶC KÉP TRẦN: Nếu bắt buộc phải dùng ngoặc kép bên trong nội dung văn bản, bạn phải thực hiện escape nghiêm ngặt bằng cách thêm dấu gạch chéo ngược ngay trước nó, ví dụ: \\"khái niệm\\". Tốt nhất là dùng ngoặc đơn '...' hoặc dấu nháy đơn để thay thế.

⚠️ CƠ CHẾ PHÒNG THỦ KHI THIẾU THÔNG TIN CHUYÊN MÔN:
- Nếu [CONTEXT] đưa vào chỉ toàn thông tin rác (trang bìa, mục lục, bản quyền, danh sách tác giả) và không có đủ nội dung chi tiết để viết bài giảng: Hãy sử dụng kiến thức khoa học chuẩn mực hiện hành của chủ đề đó (Ví dụ: Vật lý 10) để tự động xây dựng nội dung bài giảng chất lượng cao tương ứng với tiêu đề chương/bài. Tuyệt đối không để trống, không viết outline sơ sài và không sinh ra nội dung vô nghĩa từ trang bìa.

⚠️ QUY TẮC THIẾT KẾ SƯ PHẠM NGHIÊM NGẶT:
- Chia khóa học từ 3-6 chương, mỗi chương có đúng 2-3 bài học.
- Nội dung sắp xếp logic từ cơ bản đến nâng cao dựa trên tài liệu.
- Mỗi bài học PHẢI có nội dung giảng dạy chi tiết trong trường "lecture" (từ 2-4 đoạn văn diễn giải rõ ý), tuyệt đối không được để trống hoặc chỉ trả về tiêu đề (title).
- "lecture", "key_points", "activity", "assessment" phải bám sát dữ liệu trong [CONTEXT] (kết hợp tri thức khoa học chuẩn nếu tài liệu bị khuyết thiếu thông tin học thuật).
- "key_points" phải có ĐÚNG 5 ý chính, mỗi ý thuộc một dạng khác nhau theo đúng thứ tự:
  + Ý 1: Nhận biết (Thuật ngữ, khái niệm cốt lõi)
  + Ý 2: Ví dụ (Minh họa thực tế sinh động)
  + Ý 3: Liên hệ (Mối liên hệ giữa ý này với các bài học trước/sau)
  + Ý 4: Ứng dụng (Ứng dụng thực tế hoặc bài tập vận dụng ngắn)
  + Ý 5: Gợi mở (Câu hỏi gợi mở để người học tự suy luận thêm)
- "assessment" phải có ĐÚNG 3 câu hỏi thuộc 3 cấp độ khác nhau theo đúng thứ tự:
  + Câu 1: Câu hỏi tái hiện kiến thức (Nhận biết khái niệm cốt lõi)
  + Câu 2: Câu hỏi thông hiểu (Yêu cầu người học tự diễn giải lại)
  + Câu 3: Câu hỏi vận dụng (Tình huống hoặc bài toán thực tế yêu cầu áp dụng lý thuyết)
    ...
    {{
      ...
      "lessons": [
        {{
          ...
          "lecture": "Nội dung bài giảng từ 2-4 đoạn, diễn giải sâu sắc, rõ ràng về mặt kiến thức khoa học. Tuyệt đối không chứa tên file, số trang, hoặc lời khuyên đọc tài liệu gốc. Ngắt các đoạn văn bằng ký tự '\\\\n\\\\n' thay vì nhấn phím Enter xuống dòng.",
          "key_points": [
            "[Nhận biết] Ý chính học thuật giải nghĩa khái niệm cốt lõi",
            "[Ví dụ] Minh họa thực tế sinh động cho khái niệm trên",
            "[Liên hệ] Mối quan hệ của bài học này với các chủ đề khác",
            "[Ứng dụng] Cách áp dụng kiến thức vào giải quyết vấn đề",
            "[Gợi mở] Câu hỏi tư duy kích thích suy luận sâu hơn"
          ],
          "activity": "Hoạt động thực hành cụ thể, gắn liền với kiến thức bài học (ví dụ: làm thí nghiệm ảo, phân tích tình huống).",
          "assessment": [
            "[Nhận biết] Hỏi thẳng vào định nghĩa/khái niệm vừa học",
            "[Thông hiểu] Yêu cầu giải thích hiện tượng thực tế bằng lời văn tự do",
            "[Vận dụng] Bài toán/tình huống thực tế yêu cầu áp dụng công thức hoặc lý thuyết để giải quyết"
          ]
        }}
      ]
    }}
...
SLIDES_V2_PROMPT = """\
BẠN LÀ CHUYÊN GIA SƯ PHẠM CẤP CAO VÀ THIẾT KẾ TRẢI NGHIỆM HỌC TẬP (LX DESIGNER).
NHIỆM VỤ: Thiết kế nội dung bài giảng chất lượng cao gồm đúng {num_slides} trang slide về chủ đề "{topic}" từ tài liệu [CONTEXT].

⚠️ QUY TẮC PHÂN BỔ SƯ PHẠM (LOGICAL FLOW):
Hãy phân bổ đúng {num_slides} slide theo tiến trình nhận thức khoa học sau:
- Slide 1 (SLIDE GIỚI THIỆU - BẮT BUỘC): Giới thiệu lôi cuốn về chủ đề "{topic}". Tập trung trả lời câu hỏi "Tại sao kiến thức này quan trọng?" hoặc "Ứng dụng thực tế nổi bật nhất của nó là gì?". Tạo ra một cú "hook" (mồi nhử) để thu hút người học ngay lập tức.
- Các slide trung gian (Slide 2 đến slide kế cuối): Đi sâu vào định nghĩa khái niệm cốt lõi, cơ chế hoạt động, định luật hoặc phân loại chi tiết có trong [CONTEXT].
- Slide cuối cùng: Tổng kết bài học hoặc đưa ra một câu hỏi tương tác, gợi mở thực tế.

⚠️ QUY TẮC LỌC NỘI DUNG (CỰC KỲ QUAN TRỌNG):
1. TUYỆT ĐỐI BỎ QUA: Trang bìa, tên tác giả, lời nói đầu, mục lục, trang bản quyền, danh sách ban biên tập.
2. CHỈ TRÍCH XUẤT: Kiến thức học thuật cốt lõi, định nghĩa, định luật, ví dụ và ứng dụng thực tế.
3. Nếu [CONTEXT] chứa thông tin tác giả hoặc nhà xuất bản, hãy lập tức bỏ qua và nhảy đến phần kiến thức tiếp theo.

⚠️ TIÊU CHUẨN NỘI DUNG SLIDE (CHỐNG TRÀN CHỮ - QUAN TRỌNG):
- "title": Phải cực kỳ ngắn gọn (tối đa 6 từ), tập trung vào một khái niệm duy nhất của slide đó.
- "content": Viết dưới dạng Markdown gạch đầu dòng (tối đa 3 gạch đầu dòng). Mỗi gạch đầu dòng chỉ được chứa tối đa 15 từ. Loại bỏ hoàn toàn các từ nối thừa thãi. Sử dụng kỹ thuật in đậm (**Từ khóa**) để làm nổi bật thông tin quan trọng.
- "image_suggestion": Tạo prompt vẽ ảnh AI chi tiết bằng tiếng Anh theo công thức: "[Chủ thể liên quan trực tiếp đến kiến thức] + phong cách 3D minimalist icon hoặc Flat Vector Illustration + đặt trên nền trắng studio tách biệt + màu sắc hiện đại tương phản". Tuyệt đối không viết mô tả chung chung như "ảnh minh họa".

⚠️ QUY TẮC CẤU TRÚC JSON (BẮT BUỘC):
1. TUYỆT ĐỐI KHÔNG dùng dấu ngoặc kép (") bên trong phần văn bản. 
2. Nếu cần nhấn mạnh hoặc trích dẫn, hãy dùng dấu nháy đơn (') hoặc in đậm bằng dấu sao (**).
3. Đảm bảo mỗi slide là một object hoàn chỉnh, ngăn cách bằng dấu phẩy.

YÊU CẦU ĐỊNH DẠNG (CHỈ XUẤT RAW JSON ARRAY - KHÔNG KÈM TEXT GIẢI THÍCH):
[
  {{
    "title": "Tiêu đề ngắn gọn của Slide (Ví dụ: 'Định luật Newton I')",
    "content": "**Khái niệm chính:** Vật sẽ giữ nguyên trạng thái đứng yên hoặc chuyển động thẳng đều nếu không chịu lực tác động.\\n- **Hệ quả:** Xuất hiện quán tính.\\n- **Ví dụ:** Người bị ngả về phía trước khi xe phanh gấp.",
    "image_suggestion": "A 3D minimalist icon of a sliding block showing inertia force, isometric view, clean white background, vivid academic colors"
  }}
]
