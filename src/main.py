...
# Tìm đoạn này trong main.py và sửa lại như sau:
@app.post("/api/generate-slides")
async def api_generate_slides(req: GenerateSlidesRequest):
    rag = get_course(req.course_id)
    # Lấy danh sách slide dưới dạng JSON (Array of objects)
    # Chứ KHÔNG gọi generate_slides_html ở đây
    result = rag.get_resource_generator().generate_slides_v2(
        topic=req.topic, 
