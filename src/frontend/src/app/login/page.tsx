...
import { GraduationCap, Sparkles, LogIn, Mail, Lock, ArrowRight } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Đăng nhập không thành công. Vui lòng kiểm tra lại email và mật khẩu.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md space-y-8 bg-card/80 backdrop-blur-md p-8 rounded-2xl border border-border/60 shadow-xl">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Truy cập không gian làm việc AI Course Generator của bạn
          </p>
        </div>
