...
- **AI:** OpenRouter-only, paid-only, model theo từng feature. Book/Vid/OCR dùng `google/gemini-2.5-pro` (mặc định); Slide/Quiz dùng `google/gemini-2.5-flash`. Mỗi call retry đúng model đã chọn một lần khi provider/schema lỗi; embedding dùng model OpenRouter đã cấu hình riêng.
